...
<body>
    ...
    <table>
        <tr>
            ...
            <c:forEach var="things" items="${thingsList}">
                <tr>
                    <td>${things.id} </td>
                    <td>${things.name}</td>
                    <td>${things.price}</td>
                    <td><a href="/things/addBasket/${things.id}">Добавить в корзину</a></td>
                    <td><a href="/things/delete/${things.id}">Удалить товар</a></td>
                </tr>
            </c:forEach>
    </table>

    <h2>Add</h2>
    <c:url value="/things/addThings" var="add"/>
