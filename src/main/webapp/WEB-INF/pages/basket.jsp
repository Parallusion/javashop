...
<body>
    ...
    <form action="/basket" method="POST">
        <table>
            ...
            <c:forEach var="things" items="${basketMap}">
                ...
                <tr>
                    <td>${things.key.id} </td>
                    <td>${things.key.name}</td>
                    <td>${things.key.price}</td>
                    <td><input type="number"id="number" name="number" min="1" max="7" value="${things.value}"></td>
                    <td>${things.key.price * things.value}</td>
                    <td><a href="/basket/delete/${things.key.id}">Убрать</a></td>
                </tr>
            </c:forEach>
            <tr>
    ...
    <c:if test="${!empty buyer.name && !empty basketMap}">
        <h2>Оформление</h2>
        <a href="/ordered/confirm">Оформить заказ</a>
    </c:if>
